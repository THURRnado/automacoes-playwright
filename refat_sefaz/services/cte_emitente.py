...
PROCESS_DIR_TXT = "cte_emitente_txt"
PROCESS_DIR_XML = "cte_emitente_xml"
...
def _preencher_formulario(page, dt_inicio: str, dt_final: str, empresa_ie: str, tipo: str) -> tuple[str, str]:
    ...
    logger.info(f"Acessando página de consulta CT-e emitente — tipo: {tipo.upper()}")
    ...
    # Habilita o botão Consultar antes de clicar
...
def run_txt(
    ...
    """Solicita e baixa o arquivo TXT de CT-e emitente no SEFAZ."""
    logger.info(f"Iniciando CT-e emitente TXT — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante CT-e emitente TXT: {e}", exc_info=True)
        raise
...
def run_xml(
    ...
    """Solicita e baixa o arquivo ZIP com XMLs de CT-e emitente no SEFAZ."""
    logger.info(f"Iniciando CT-e emitente XML — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante CT-e emitente XML: {e}", exc_info=True)
        raise
