...
CONSULTA_URL = (
    "https://www4.sefaz.pb.gov.br/atf/fis/FISf_ConsultaGenericaEmitenteNFCe.do"
    "?idSERVirtual=S&h=https://www.sefaz.pb.gov.br/ser/servirtual/credenciamento/info"
)

PROCESS_DIR_TXT = "nfce_destinatario_txt"
PROCESS_DIR_XML = "nfce_destinatario_xml"

DEFAULT_DT_INICIO  = "01/04/2026"
DEFAULT_DT_FINAL   = "30/04/2026"
DEFAULT_EMPRESA_IE = "163393320"


def _preencher_formulario(page, dt_inicio: str, dt_final: str, empresa_ie: str, tipo: str) -> tuple[str, str]:
    """
    Preenche o formulário de consulta NFC-e destinatário e submete.
    ...
    logger.info(f"Acessando página de consulta NFC-e destinatário — tipo: {tipo.upper()}")
    ...
    click(page, 'button[name="btnConsultar"]')
...
def run_txt(
    ...
    """Solicita e baixa o arquivo TXT de NFC-e destinatário no SEFAZ."""
    logger.info(f"Iniciando NFC-e destinatário TXT — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante NFC-e destinatário TXT: {e}", exc_info=True)
        raise
...
def run_xml(
    ...
    """Solicita e baixa o arquivo ZIP com XMLs de NFC-e destinatário no SEFAZ."""
    logger.info(f"Iniciando NFC-e destinatário XML — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante NFC-e destinatário XML: {e}", exc_info=True)
        raise
