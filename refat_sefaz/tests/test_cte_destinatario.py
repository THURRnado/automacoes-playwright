# tests/test_cte_destinatario.py
import os
import shutil
import pytest
import sys
...
from services.cte_destinatario import (
    run_txt, run_xml,
    DEFAULT_DT_INICIO, DEFAULT_DT_FINAL, DEFAULT_EMPRESA_IE,
    PROCESS_DIR_TXT, PROCESS_DIR_XML,
)
...
class TestCTeDestinatarioTXT:
    ...
    def test_salvo_no_diretorio_correto(self, cleanup_downloads):
        file_path = run_txt()
        cleanup_downloads.append(os.path.dirname(os.path.dirname(file_path)))
        assert PROCESS_DIR_TXT in file_path, f"Arquivo não está na pasta correta: {file_path}"
        assert DOWNLOAD_BASE_PATH in file_path, f"Arquivo fora do caminho base esperado: {file_path}"

    def test_parametros_customizados(self, cleanup_downloads):
        file_path = run_txt(dt_inicio=DEFAULT_DT_INICIO, dt_final=DEFAULT_DT_FINAL, empresa_ie=DEFAULT_EMPRESA_IE)
        cleanup_downloads.append(os.path.dirname(os.path.dirname(file_path)))
...
class TestCTeDestinatarioXML:
    ...
    def test_salvo_no_diretorio_correto(self, cleanup_downloads):
        file_path = run_xml()
        cleanup_downloads.append(os.path.dirname(os.path.dirname(file_path)))
        assert PROCESS_DIR_XML in file_path, f"Arquivo não está na pasta correta: {file_path}"
        assert DOWNLOAD_BASE_PATH in file_path, f"Arquivo fora do caminho base esperado: {file_path}"
