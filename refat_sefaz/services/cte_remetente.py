...
PROCESS_DIR_TXT = "cte_remetente_txt"
PROCESS_DIR_XML = "cte_remetente_xml"
...
def _preencher_formulario(page, dt_inicio: str, dt_final: str, empresa_ie: str, tipo: str) -> tuple[str, str]:
    ...
    logger.info(f"Acessando página de consulta CT-e remetente — tipo: {tipo.upper()}")
    ...
    logger.debug(f"Preenchendo IE no iframe do remetente: {empresa_ie}")
    reme_frame = page.frame_locator('iframe[name="cmpReme"]').first
    reme_frame.locator('input[name="hidNrDocumentocmpReme"]').fill(empresa_ie)
    reme_frame.locator('input[name="btnPesquisar"]').click()

    logger.debug("Aguardando preenchimento da Razão Social")
    page.wait_for_function(
        """() => {
            const iframe = document.querySelector('iframe[name="cmpReme"]');
            if (!iframe) return false;
            const campo = iframe.contentDocument.querySelector('input[name="hidNoHumanoInstcmpReme"]');
...
def run_txt(
    ...
    """Solicita e baixa o arquivo TXT de CT-e remetente no SEFAZ."""
    logger.info(f"Iniciando CT-e remetente TXT — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante CT-e remetente TXT: {e}", exc_info=True)
        raise
...
def run_xml(
    ...
    """Solicita e baixa o arquivo ZIP com XMLs de CT-e remetente no SEFAZ."""
    logger.info(f"Iniciando CT-e remetente XML — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante CT-e remetente XML: {e}", exc_info=True)
        raise
