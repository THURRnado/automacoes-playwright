...
PROCESS_DIR_TXT = "nfce_emitente_txt"
PROCESS_DIR_XML = "nfce_emitente_xml"
...
def _preencher_formulario(page, dt_inicio: str, dt_final: str, empresa_ie: str, tipo: str) -> tuple[str, str]:
    """
    Preenche o formulário de consulta NFC-e emitente e submete.
    ...
    logger.info(f"Acessando página de consulta NFC-e emitente — tipo: {tipo.upper()}")
...
def run_txt(
    ...
    """Solicita e baixa o arquivo TXT de NFC-e emitente no SEFAZ."""
    logger.info(f"Iniciando NFC-e emitente TXT — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante NFC-e emitente TXT: {e}", exc_info=True)
        raise
...
def run_xml(
    ...
    """Solicita e baixa o arquivo ZIP com XMLs de NFC-e emitente no SEFAZ."""
    logger.info(f"Iniciando NFC-e emitente XML — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante NFC-e emitente XML: {e}", exc_info=True)
        raise
