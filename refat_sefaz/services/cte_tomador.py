...
PROCESS_DIR_TXT = "cte_tomador_txt"
PROCESS_DIR_XML = "cte_tomador_xml"
...
def _preencher_formulario(page, dt_inicio: str, dt_final: str, empresa_ie: str, tipo: str) -> tuple[str, str]:
    ...
    logger.info(f"Acessando página de consulta CT-e tomador — tipo: {tipo.upper()}")
    ...
    logger.debug(f"Preenchendo IE no iframe do tomador: {empresa_ie}")
    toma_frame = page.frame_locator('iframe[name="cmpToma"]').first
    toma_frame.locator('input[name="hidNrDocumentocmpToma"]').fill(empresa_ie)
    toma_frame.locator('input[name="btnPesquisar"]').click()

    logger.debug("Aguardando preenchimento da Razão Social")
    page.wait_for_function(
        """() => {
            const iframe = document.querySelector('iframe[name="cmpToma"]');
            if (!iframe) return false;
            const campo = iframe.contentDocument.querySelector('input[name="hidNoHumanoInstcmpToma"]');
...
def run_txt(
    ...
    """Solicita e baixa o arquivo TXT de CT-e tomador no SEFAZ."""
    logger.info(f"Iniciando CT-e tomador TXT — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante CT-e tomador TXT: {e}", exc_info=True)
        raise
...
def run_xml(
    ...
    """Solicita e baixa o arquivo ZIP com XMLs de CT-e tomador no SEFAZ."""
    logger.info(f"Iniciando CT-e tomador XML — IE: {empresa_ie} | {dt_inicio} a {dt_final}")
    ...
    try:
        ...
        logger.error(f"Erro durante CT-e tomador XML: {e}", exc_info=True)
        raise
